...
resource "aws_instance" "LAMPNode01" {
  ...
  connection {
    user = "${var.LAMPNode01-os_admin_user == "" ? lookup(var.default_os_admin_user, format("%s_%s", replace(var.LAMPNode01-image, "/", "_"), var.aws_ami_owner_id)) : var.LAMPNode01-os_admin_user}"
...
resource "camc_bootstrap" "LAMPNode01_chef_bootstrap_comp" {
  ...
  data = <<EOT
{
  "os_admin_user": "${var.LAMPNode01-os_admin_user == "default"? lookup(var.default_os_admin_user, format("%s_%s", replace(var.LAMPNode01-image, "/", "_"), var.aws_ami_owner_id)) : var.LAMPNode01-os_admin_user}",
